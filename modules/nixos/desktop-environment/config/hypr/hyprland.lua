...
hl.bind("F11", hl.dsp.exec_cmd("pidof -x \"$(dirname $(readlink -f $(which hyprshot)))/.hyprshot-wrapped\" &>/dev/null || hyprshot -m window --clipboard-only"))
-- Bind print screen key for printing a selected region and copying to clipboard
hl.bind("SHIFT + F11", hl.dsp.exec_cmd("pidof -x \"$(dirname $(readlink -f $(which hyprshot)))/.hyprshot-wrapped\" &>/dev/null || hyprshot -m region --clipboard-only"))
